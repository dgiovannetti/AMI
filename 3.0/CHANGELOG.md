...
- **macOS Dock da sorgente** (`python -m ami.main`): subito dopo `QApplication`, **`setWindowIcon`** con `resources/ami.icns` o `ami.png` — senza bundle macOS mostrerebbe l’icona di Python nel Dock.
...
- **macOS menu bar tray — icone previste**: di nuovo **PNG a colori** ufficiali (`status_green|yellow|red.png`, ✓ / ! / ✕) con più tagli **1x e @2x** (`devicePixelRatio`). La maschera template resta solo con **`AMI_TRAY_TEMPLATE=1`** se servisse un fallback. Raster 512 solo se manca il file.
...
- **macOS tray / Dock / avvio**: `LSUIElement` **false** (icona **Dock**). **Click** sull’icona tray apre **un solo** menu (`setContextMenu`; rimosso `popup()` extra su `Trigger` che duplicava il menu). **Splash** non insieme alla finestra compatta; default **`compact_status_window`: false**. **Compact**: chiudi = **nascondi**; Dock riapre compatta o dashboard se non ci sono finestre.
- **Risorsa `status_green.png`**: la PNG precedente mostrava solo una barra diagonale; sostituita con cerchio verde **#10B981** e **spunta bianca** a due segmenti (disegnata ad alta risoluzione e scalata a 64×64 per nitidezza in menu bar).
