...
- **macOS crash Qt / CoreFoundation**: Su macOS recenti, Qt 6 poteva andare in **SIGSEGV** in `CFBundleCopyBundleURL` durante il caricamento di `QtCore.abi3.so` (QLibraryInfo senza plugin path nel bundle PyInstaller). Fix: **`ami._macos_qt_env.apply_if_needed()`** prima di importare PyQt6 in `main.py` + runtime hook **`pyi_rth_pyqt6_macos.py`** che imposta `QT_PLUGIN_PATH`, `QT_QPA_PLATFORM_PLUGIN_PATH`, `QT_QPA_PLATFORM=cocoa` cercando sotto `_MEIPASS` e `Contents/Frameworks`.
