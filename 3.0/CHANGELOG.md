...
- **macOS packaging / Gatekeeper**: Build ufficiale come **`AMI.app`** (spec `ami_macos.spec`) con `Info.plist` (`CFBundleName` **AMI**, bundle id `tech.ciaoim.ami`). **`codesign`** ad-hoc (`-`, `--timestamp=none`) su **ogni Mach-O** nel bundle poi sull’`.app` (meglio del solo `--deep`). Avviso **«Python»** in Gatekeeper: spesso si è aperto **`Contents/MacOS/AMI`** invece dell’icona **AMI.app** — in pacchetto **`LEGGIMI_macOS.txt`** e in **`MACOS_SECURITY.md`**. Esclusi pacchetti opzionali pesanti (`scipy`, ecc.) dalla Analysis. Variabile **`AMI_CODESIGN_IDENTITY`** per Developer ID. **Release CI**: rimozione asset legacy `AMI-macOS.zip` / `AMI-Windows.zip` prima dell’upload. **OTA su macOS**: sostituzione intero `AMI.app` + `open -n`.
