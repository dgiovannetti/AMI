...
- **macOS menu bar tray — icone previste**: sempre le PNG **`resources/status_*.png`**. Su macOS il sistema spesso **non disegna PNG a colori** nel tray: si genera una **maschera template** dalla stessa immagine (alpha + nero, `setIsMask`), così la **forma è quella ufficiale** e l’icona è visibile. Fallback a PNG colori scalate; `AMI_TRAY_COLOR=1` forza solo il percorso a colori. Raster 512 solo se manca il file.
