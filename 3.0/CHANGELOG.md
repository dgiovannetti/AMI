...
- **macOS menu bar tray — icone previste**: di nuovo **PNG a colori** ufficiali (`status_green|yellow|red.png`, ✓ / ! / ✕). Su macOS **`NSStatusItem` nativo** (AppKit) perché `QSystemTrayIcon` spesso non disegna PNG a colori (geometry ok, icona invisibile). Fallback Qt con **`AMI_TRAY_QT=1`**; maschera template con **`AMI_TRAY_TEMPLATE=1`**.
