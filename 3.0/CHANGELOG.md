...
- **macOS tray .app geometry 0×h**: il primo `show()` sincrono in `__init__` poteva lasciare `QSystemTrayIcon.geometry()` con **altezza 0** (icona invisibile). Fix: primo `show()` su **QTimer(0)** nel frozen; più reassert; `QIcon` da **path file**; fallback cerchi multi‑size senza `devicePixelRatio`. Variabile **`AMI_DEBUG_TRAY=1`** stampa diagnostica su stderr.
...
- **macOS menu bar / tray** (storico 3.1.4 iniziale): poi rivisto in **Unreleased** — `LSUIElement` ripristinato **false**, icone template, click tray, Dock, compact/splash.
