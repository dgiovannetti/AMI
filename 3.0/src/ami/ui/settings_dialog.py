...
class SettingsDialog(QDialog):
    ...
    def _init_speed_test_tab(self) -> None:
        ...
        self.speed_test_warmup_mb = QDoubleSpinBox()
        self.speed_test_warmup_mb.setRange(0, 20)
        self.speed_test_warmup_mb.setDecimals(1)
        self.speed_test_warmup_mb.setValue(float(st.get("warmup_mb", 2)))
        self.speed_test_warmup_mb.setSuffix(" MB")
        self.speed_test_warmup_mb.setToolTip(
            "Bytes read before timing (TCP slow start). 0 disables. Not counted in speed."
        )
        layout.addRow("Warmup (not timed):", self.speed_test_warmup_mb)
        ...
        self.speed_test_size_mb.setToolTip("Timed download size after warmup (total traffic = warmup + this).")
        layout.addRow("Timed download size:", self.speed_test_size_mb)
