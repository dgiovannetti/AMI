...
- **macOS (3.1.4+):** the ZIP contains **`AMI.app`** at archive root (or under `AMI-Package/` in older builds). The updater uses `rglob` to find **`AMI.app`**, replaces the **entire bundle**, and relaunches with `open -n`. Older flat `AMI` binaries are still supported if no `AMI.app` is present.
